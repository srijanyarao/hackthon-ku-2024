...
out_labels=['A','B','C','D','E','F','G','H','I','K','L','M','N','O','P','Q','R','S','T','U','V','W','X','Y'] # labels
...
def detection(frame):
    ...
    if results.multi_hand_landmarks: # if hand is detected
        ...
        for hand_landmarks in results.multi_hand_landmarks: # for each detected hand image
            ...
            mp_drawing.draw_landmarks(frame, hand_landmarks, mp_hands.HAND_CONNECTIONS) # mark landmarks in live feed
            break
...
def preprocess(frame):
    ...
    gray_image = cv2.GaussianBlur(gray_image, (7, 7), 0) # gaussian blur to smoothen
    gray_image = cv2.flip(gray_image, 1) # flip image to original for prediction
    gray_image = cv2.resize(gray_image, (28, 28)) # convert to a 28*28 image. Dataset images had same size
...
while True:
    ...
    [y, frame2] = detection(frame) # detect hand, draw a bounding box, mark landmarks and crop the hand image
    ...
    if y and frame2.any() != 0:
        ...
        cv2.putText(frame,image,(10,90),font_style,5,(0,0,255),3) # Annotate on live feed
